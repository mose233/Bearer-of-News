...
function VideoTemplatePanel({
  ...
  const addStagedVideoMediaToTimeline = () => {
    ...
    setVideoDraftStatus(
      `${tool} ${selectedVideoDuration} video is being generated by fal.ai...`
    );
    ...
    try {
      ...
      if (result.status === "failed" || !result.videoUrl) {
        throw new Error(
          result.error || "fal.ai did not return a video."
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          `Could not download the fal.ai video (${response.status}).`
        );
      }
      ...
      setVideoDraftStatus(
        `${tool} ${selectedVideoDuration} real AI video generated by fal.ai. It has been added to the preview and timeline.`
      );

      onGenerateCompleteVideo?.();
    } catch (error) {
      console.error("Video AI fal.ai generation failed:", error);
