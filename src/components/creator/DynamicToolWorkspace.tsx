...
function VideoTemplatePanel({
  ...
  useEffect(() => {
    return () => {
      if (generatedVideoPreview) URL.revokeObjectURL(generatedVideoPreview);
    };
 ...
 const buildDraftPrompt = () => {
  ...
  return;
    ...
    if (generatedVideoPreview) {
      URL.revokeObjectURL(generatedVideoPreview);
      setGeneratedVideoPreview("");
    }

    setIsGeneratingVideo(true);
    setVideoDraftStatus(
      `${tool} ${selectedVideoDuration} video is being generated by xnewsapp.com...`
    );
  ...
  aspectRatio: "9:16",
      ...
      if (result.status === "failed" || !result.videoUrl) {
        throw new Error(
          result.error || "The AI service did not return a video."
        );
      }
      ...
      const file = new File(
        [blob],
        `xnewsapp-${tool.toLowerCase().replace(/\s+/g, "-")}-ai-video.mp4`,
        {
          type: "video/mp4",
        }
      );
      ...
      if (onAddEnhancedPhotoToTimeline) {
        onAddEnhancedPhotoToTimeline(
          file,
          preview,
          getDurationSecondsFromLabel(selectedVideoDuration)
        );
      }

      setVideoDraftStatus(
        `${tool} ${selectedVideoDuration} video is ready. It has been added to the preview and timeline.`
      );
...
/>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/80">
          <div className="border-b border-white/10 px-4 py-3">
            <h3 className="text-base font-extrabold text-white">Preview</h3>
          </div>

          <div className="flex min-h-[300px] items-center justify-center p-5 text-center">
            {isGeneratingVideo ? (
              <div className="w-full max-w-md space-y-5">
                <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full border-8 border-white/10 border-t-violet-400 border-r-blue-400 animate-spin">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm font-medium leading-6 text-slate-300">
                    Please wait while xnewsapp.com creates your video.
                    <br />
                    This may take a few moments...
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm font-medium leading-6 text-slate-300">
                  💡 Tip: You can keep this page open. We’ll show your video here as soon as it’s ready.
                </div>
              </div>
            ) : generatedVideoPreview ? (
              <div className="w-full space-y-3">
                ...
                <video
                  ...
                  controls
                  playsInline
                  className="mx-auto max-h-[520px] w-full rounded-2xl bg-black object-contain"
                />
              </div>
            ) : (
  ...
  onClick={() => {
    if (requestGeneration) {
      requestGeneration("$0.70", () => handlePrepareDraft(true));
    } else {
      handlePrepareDraft(true);
    }
  }}
/>
  ...
  const handleGenerateCinematicTextToVideo = async () => {
    ...
    setCinematicStatus(
      `${tool} ${selectedCinematicDuration} real AI video is being generated by fal.ai...`
    );
    ...
    try {
      ...
      const aspectRatio =
        cinematicAspectRatio === "9:16 Reel"
          ? "9:16"
          : cinematicAspectRatio === "1:1 Square"
            ? "1:1"
            : "16:9";
      ...
      const result = await generateFalVideo({
        ...
        durationSeconds: getDurationSecondsFromLabel(
          selectedCinematicDuration
        ),
        aspectRatio,
      });
      ...
      if (result.status === "failed" || !result.videoUrl) {
        throw new Error(
          result.error || "fal.ai did not return a cinematic video."
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          `Could not download the fal.ai cinematic video (${response.status}).`
        );
      }
      ...
      setCinematicStatus(
        `${tool} ${selectedCinematicDuration} real AI video generated by fal.ai. It has been added to the preview and timeline.`
      );

      onGenerateCompleteVideo?.();
    } catch (error) {
      console.error(
        "Cinematic Text to Video fal.ai generation failed:",
        error
      );

      setCinematicStatus(
        `Cinematic video generation failed: ${
          error instanceof Error
            ? error.message
   ...
   const handleGenerateCinematicDraft = async () => {
    if (!stagedCinematicFile) {
      setCinematicStatus(
        "Please upload an image before generating a cinematic AI video."
      );
      return;
    }

    if (!stagedCinematicFile.type.startsWith("image/")) {
      setCinematicStatus(
        "Cinematic AI currently requires an image as the source media."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "Cinematic AI fal.ai generation failed:",
        error
      );
    ...
    const handleGenerateCinematicTalkingAvatar = async () => {
  if (!stagedCinematicFile) {
    setCinematicStatus(
      "Please upload an avatar or presenter image before generating a Talking Avatar."
    );
    return;
  }
  ...
  if (!stagedCinematicFile.type.startsWith("image/")) {
    setCinematicStatus(
      "Talking Avatar requires an image as the avatar or presenter source."
    );
    return;
  }
  ...
  if (!aiVoiceBlob) {
    setCinematicStatus(
      "Please generate the Avatar Script voice before generating the Talking Avatar."
    );
    return;
  }
  ...
  try {
    const { generateFalVideo } = await import(
      "@/lib/fal/falService"
    );
    ...
    console.log(
      "Starting real fal.ai Talking Avatar generation:",
      {
        tool: "Talking Avatar",
        hasAvatarImage: Boolean(stagedCinematicFile),
        avatarImageType: stagedCinematicFile.type,
        avatarImageSize: stagedCinematicFile.size,
        hasVoiceBlob: Boolean(aiVoiceBlob),
        voiceMimeType: aiVoiceBlob.type,
        voiceSize: aiVoiceBlob.size,
        durationSeconds:
          getDurationSecondsFromLabel(
            selectedCinematicDuration
          ),
        aspectRatio,
      }
    ...
    console.log(
      "Talking Avatar fal.ai generation result:",
      result
    );
    ...
    if (result.status === "failed" || !result.videoUrl) {
      throw new Error(
        result.error ||
          "fal.ai did not return a Talking Avatar video."
      );
    }
    ...
    if (!response.ok) {
      throw new Error(
        `Could not download the fal.ai Talking Avatar video (${response.status}).`
      );
    }
    ...
    const file = new File(
      [blob],
      "xnewsapp-talking-avatar-ai-video.mp4",
      {
        type: "video/mp4",
      }
    );
    ...
    console.error(
      "Cinematic Talking Avatar fal.ai generation failed:",
      error
    );
    ...
    setCinematicStatus(
      `Talking Avatar generation failed: ${
        error instanceof Error
          ? error.message
          : "fal.ai Talking Avatar generation failed."
      }`
    );
  }
};
...
const isMemeGenerator = tool === "Meme Generator";
  ...
  : isAIArtGenerator
  ? "Describe the artwork you want AI to generate..."
  : isFacebookPostImage
  ? "Describe the Facebook post image..."
  : isInstagramPostImage
  ? "Describe the Instagram post image..."
  : isWhatsAppStatusImage
  ? "Describe the WhatsApp status image..."
  : isPosterFlyer
  ? "Describe your poster or flyer..."
  : isEventPoster
  ? "Describe your event poster..."
  : isBusinessBanner
  ? "Describe your business banner..."
  : isProductAdImage
